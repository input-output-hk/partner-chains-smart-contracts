...
import Contract.Prelude (class Generic, class Newtype, type (/\), Unit, Void, bind, discard, null, show, unwrap, when, zero, ($), (/=), (/\), (<$>), (<<<), (<>), (=<<), (==), (>>>))
...
import Contract.Transaction (TransactionInput, TransactionOutput(..), awaitTxConfirmed, balanceAndSignTx, submit)
import Contract.TxConstraints as Constraints
import Contract.ScriptLookups as Lookups
import Contract.PlutusData (class FromData, class ToData, PlutusData(Constr), fromData, toData)
import Contract.Address (PubKeyHash, getNetworkId, validatorHashEnterpriseAddress)
...
newtype UpdateCommitteeHashDatum = UpdateCommitteeHashDatum { committeeHash ∷ String }
...
instance ToData UpdateCommitteeHashDatum where
  toData (UpdateCommitteeHashDatum { committeeHash }) = Constr zero [ toData committeeHash ]
instance FromData UpdateCommitteeHashDatum where
  fromData (Constr n [ a ])
    | n == zero = UpdateCommitteeHashDatum <$> ({ committeeHash: _}) <$> fromData a
  fromData _ = Nothing
...
newtype UpdateCommitteeHash = UpdateCommitteeHash { uchAssetClass ∷ Value.CurrencySymbol /\ Value.TokenName }
...
instance ToData UpdateCommitteeHash where
  toData (UpdateCommitteeHash { uchAssetClass })
    = Constr zero [ toData uchAssetClass ]

newtype InitCommitteeHashMint = InitCommitteeHashMint { icTxOutRef ∷ TransactionInput }
...
instance ToData UpdateCommitteeHashParams where
  toData (UpdateCommitteeHashParams { sidechainParams , newCommitteePubKeys , newCommitteeSignatures , committeePubKeys })
    = Constr zero [ toData sidechainParams , toData newCommitteePubKeys , toData newCommitteeSignatures
                  , toData committeePubKeys ]

committeeHashPolicy ∷ InitCommitteeHashMint → Contract () MintingPolicy
committeeHashPolicy sp = do
  policyUnapplied ← (plutusV2Script >>> MintingPolicy) <$> textEnvelopeBytes rawUpdateCommitteeHash PlutusScriptV2
...
updateCommitteeHash (UpdateCommitteeHashParams uchp) = do
  ...
  scriptUtxos ← Map.toUnfoldable <<< unwrap <$> liftedM "Cannot get script utxos" (utxosAt valAddr)
    ∷ Contract () (Array (TransactionInput /\ TransactionOutput))
  let uchCS /\ uchTN = uch.uchAssetClass
      findOwnValue (_tIN /\ tOUT) = Value.valueOf ((unwrap tOUT).amount) uchCS uchTN == fromInt 1
...
--datum  ← liftContractM "no datum" =<< liftContractM "no datum"  ((fromData <<< unwrap) <$> (dat `Map.lookup` datums))
  ...
  let newDatum = Datum $ toData (UpdateCommitteeHashDatum { committeeHash : newCommitteeHash })
      ...
      lookups     = Lookups.unspentOutputs (Map.singleton oref (TransactionOutput tOut))
