...
import Contract.Prelude (class Generic, class Newtype, Maybe(..), Unit, Void, bind, discard, flip, liftAff, mconcat, pure, show, snd, unwrap, zero, ($), (&&), (/\), (<$>), (<*>), (<<<), (<>), (=<<), (==), (>>>))
import Contract.Log (logInfo')
import Contract.Monad (Contract, liftContractM, liftedE, liftedM)
import Contract.Scripts (Validator(..), PlutusScript(..), applyArgs , validatorHash)
import Contract.TextEnvelope (TextEnvelopeType(..), textEnvelopeBytes)
import Contract.Value as Value
import Contract.Transaction ( awaitTxConfirmed , balanceAndSignTx , submit , TransactionInput , TransactionOutput(TransactionOutput))
import Contract.TxConstraints as Constraints
import Contract.ScriptLookups as Lookups
import Contract.PlutusData (class ToData , class FromData , toData , fromData , Datum(..) , PlutusData(Constr) , unitRedeemer , getDatumsByHashes)
import Contract.Address (PaymentPubKeyHash, getNetworkId, ownPaymentPubKeyHash, ownStakePubKeyHash, payPubKeyHashBaseAddress, validatorHashBaseAddress)
...
getCommitteeCandidateValidator sp = do
  ccvUnapplied ← (PlutusScript >>> Validator) <$> textEnvelopeBytes committeeCandidateValidator PlutusScriptV1
  liftedE (applyArgs ccvUnapplied [ toData sp ])

newtype UpdateCommitteeHashRedeemer = UpdateCommitteeHashRedeemer
  { committeeSignatures ∷ Array String -- | The current committee's signatures for the 'newCommitteeHash'
  , committeePubKeys    ∷ Array PaymentPubKeyHash -- | 'committeePubKeys' is the current committee public keys
...
instance ToData UpdateCommitteeHashRedeemer where
  toData (UpdateCommitteeHashRedeemer { committeeSignatures , committeePubKeys , newCommitteeHash })
    = Constr zero [ toData committeeSignatures , toData committeePubKeys , toData newCommitteeHash ]

newtype UpdateCommitteeHashParams = UpdateCommitteeHashParams
  { newCommitteePubKeys ∷ Array PaymentPubKeyHash  -- The public keys of the new committee.
  , token               ∷ AssetClass    -- The asset class of the NFT for this committee hash
  , committeeSignatures ∷ Array String  -- The signature for the new committee hash.
  , committeePubKeys    ∷ Array PaymentPubKeyHash  -- Public keys of the current committee members.
  }

-- | Endpoint parameters for initializing the committee hash
newtype GenesisCommitteeHashParams = GenesisCommitteeHashParams
  { genesisCommitteePubKeys ∷ Array PaymentPubKeyHash -- Public keys of the initial committee members.
...
newtype SaveRootParams = SaveRootParams
  ...
  , committeePubKeys ∷ Array PaymentPubKeyHash -- Public keys of all committee members
  }
...
instance ToData BlockProducerRegistration where
  toData (BlockProducerRegistration { bprSpoPubKey, bprSidechainPubKey, bprSpoSignature, bprSidechainSignature, bprInputUtxo, bprOwnPkh}) = Constr zero [toData bprSpoPubKey, toData bprSidechainPubKey, toData bprSpoSignature, toData bprSidechainSignature, toData bprInputUtxo, toData bprOwnPkh]
instance FromData BlockProducerRegistration where
  fromData (Constr n [ a , b , c , d , e , f ])
    | n == zero = BlockProducerRegistration <$> (({bprSpoPubKey: _ , bprSidechainPubKey: _ , bprSpoSignature: _ , bprSidechainSignature: _ , bprInputUtxo: _ , bprOwnPkh: _ }) <$> fromData a <*> fromData b <*> fromData c <*> fromData d <*> fromData e <*> fromData f)
  fromData _ = Nothing
...
register (RegisterParams {sidechainParams, spoPubKey, sidechainPubKey, spoSig, sidechainSig, inputUtxo}) = do
  ...
  netId     ← getNetworkId
  ownAddr   ← liftContractM "Cannot get user address" $ payPubKeyHashBaseAddress netId ownPkh ownStake
  ...
  valHash   ← liftedM "couldn't get validator hash" $ liftAff (validatorHash validator)

  let val = Value.lovelaceValueOf (BigInt.fromInt 1)
...
deregister (DeregisterParams {sidechainParams, spoPubKey}) = do
  ...
  validator ← getCommitteeCandidateValidator sidechainParams
  valHash   ← liftedM "couldn't get validator hash" (liftAff (validatorHash validator))
  valAddr   ← liftContractM "marketPlaceListNft: get validator address" (validatorHashBaseAddress netId valHash)
  ...
  ourDatums ← liftAff $ (flip parTraverse) valUtxos' $ \(input /\ (TransactionOutput out)) → runMaybeT $ do
    BlockProducerRegistration datum ← MaybeT $ pure $ (fromData <<< unwrap) =<< (_ `Map.lookup` datums) =<< out.dataHash
    guard (datum.bprSpoPubKey == spoPubKey && ownPkh == datum.bprOwnPkh)
    pure input

  let lookups ∷ Lookups.ScriptLookups Void
      lookups = Lookups.validator validator
             <> Lookups.unspentOutputs ownUtxos
             <> Lookups.unspentOutputs valUtxos
      constraints ∷ Constraints.TxConstraints Void Void
      constraints = mconcat ((\x → Constraints.mustSpendScriptOutput x unitRedeemer) <$> catMaybes ourDatums)
