...
fuelMintingPolicy sp = do
  fuelMPUnapplied ← (PlutusScript >>> MintingPolicy) <$> textEnvelopeBytes fUELMintingPolicy PlutusScriptV1
...
runFuelMP fp sp = do
  ...
  cs     ← liftContractAffM "Cannot get currency symbol" (Value.scriptCurrencySymbol fuelMP)
  logInfo' ("fuelMP curreny symbol: " <> show cs)
  tn     ← liftContractM    "Cannot get token name"      (Value.mkTokenName =<< byteArrayFromAscii "FUEL")
  let mkValue i = Value.singleton cs tn (BigInt.fromInt i)
      constraints ∷ Constraints.TxConstraints Void Void
      constraints = case fp of
        Burn bp → let redeemer = wrap (toData (MainToSide bp.recipient bp.sidechainSig))
          in Constraints.mustMintValueWithRedeemer redeemer (mkValue (-bp.amount))
        Mint mp → let value = mkValue mp.amount
          in Constraints.mustMintValueWithRedeemer (wrap (toData SideToMain)) value
