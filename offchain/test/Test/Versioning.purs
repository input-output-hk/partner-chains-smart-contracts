...
testInsertAndInvalidateSuccessScenario =
  Mote.Monad.test "Inserting new version, then invalidate the old one"
    ...
    $ \alice →
        withUnliftApp "Test.Versioning.testInsertAndInvalidateSuccessScenario"
...
testInsertSameScriptTwiceSuccessScenario =
  Mote.Monad.test "Insert same script with the same version twice"
    ...
    $ \alice → withUnliftApp
        "Test.Versioning.testInsertSameScriptTwiceSuccessScenario"
...
testInsertUnversionedScriptSuccessScenario =
  Mote.Monad.test "Insert an unversioned script"
    ...
    $ \alice → withUnliftApp
        "Test.Versioning.testInsertUnversionedScriptSuccessScenario"
        (Wallet.withKeyWallet alice)
        do
          pkh ← getOwnPaymentPubKeyHash
          genesisUtxo ← getOwnTransactionInput
...
testRemovingTwiceSameScriptFailScenario =
  Mote.Monad.test "Removing the same script twice should fail"
    ...
    $ \alice → withUnliftApp
        "Test.Versioning.testRemovingTwiceSameScriptFailScenario"
        (Wallet.withKeyWallet alice)
        do
          pkh ← getOwnPaymentPubKeyHash
          genesisUtxo ← getOwnTransactionInput
...
testRemovingScriptInsertedMultipleTimesSuccessScenario =
  Mote.Monad.test "Removing the same script twice should fail"
    ...
    $ \alice → withUnliftApp
        "Test.Versioning.testRemovingScriptInsertedMultipleTimesSuccessScenario"
        (Wallet.withKeyWallet alice)
        do
          pkh ← getOwnPaymentPubKeyHash
          genesisUtxo ← getOwnTransactionInput
...
testInsertScriptsPresentInPreviousVersion =
  Mote.Monad.test "Insert new version of a script only if present in version - 1"
    ...
    $ \alice →
        withUnliftApp "Test.Versioning.testInsertScriptsPresentInPreviousVersion"
          (Wallet.withKeyWallet alice) $ do
          pkh ← getOwnPaymentPubKeyHash
          genesisUtxo ← getOwnTransactionInput
          let
...
assertNumberOfActualVersionedScripts ∷
  ...
  Int →
  Run
    ( EXCEPT OffchainError + TRANSACTION + WALLET + READER Env + AFF + EFFECT
        + LOG
        + r
    )
    Unit
