...
testScenario1 = Mote.Monad.test "Simple update committee hash"
  ...
  $ \alice → withUnliftApp "Test.UpdateCommitteeHash.testScenario1"
...
testScenario2 =
  Mote.Monad.test "Update committee hash without honest majority (should fail)"
    ...
    $ \alice →
        withUnliftApp "Test.UpdateCommitteeHash.testScenario2"
...
testScenario3 =
  Mote.Monad.test "Update committee hash with out of order committee"
    ...
    $ \alice → withUnliftApp "Test.UpdateCommitteeHash.testScenario3"
        (Wallet.withKeyWallet alice)
        do
...
testScenario4 =
  Mote.Monad.test "Unsorted committee members (issue #277)"
    ...
    $ \alice →
        withUnliftApp "Test.UpdateCommitteeHash.testScenario4"
          (Wallet.withKeyWallet alice) $ do
          liftContract $ logInfo' "UpdateCommitteeHash 'testScenario3'"
          genesisUtxo ← Test.Utils.getOwnTransactionInput
...
testScenario5 =
  Mote.Monad.test
    ...
    $ \alice →
        withUnliftApp "Test.UpdateCommitteeHash.testScenario5"
          (Wallet.withKeyWallet alice) $ do
          liftContract $ logInfo' "UpdateCommitteeHash 'testScenario2'"
          genesisUtxo ← Test.Utils.getOwnTransactionInput
