...
testScenarioSuccess = Mote.Monad.test "Claiming FUEL tokens"
  ...
  $ \alice → withUnliftApp "Test.FUELMintingPolicy.V1.testScenarioSuccess"
...
testScenarioSuccess2 =
  Mote.Monad.test "Claiming FUEL tokens with fast distributed set lookup"
    ...
    $ \alice → withUnliftApp "Test.FUELMintingPolicy.V1.testScenarioSuccess2"
        (Wallet.withKeyWallet alice)
        do
...
testScenarioFailure =
  Mote.Monad.test "Attempt to claim with invalid merkle proof (should fail)"
    ...
    $ \alice →
        withUnliftApp "Test.FUELMintingPolicy.V1.testScenarioFailure"
          (Wallet.withKeyWallet alice)
          do
...
testScenarioFailure2 = Mote.Monad.test "Attempt to double claim (should fail)"
  ...
  $ \alice →
      withUnliftApp "Test.FUELMintingPolicy.V1.testScenarioFailure2"
        (Wallet.withKeyWallet alice)
        do
