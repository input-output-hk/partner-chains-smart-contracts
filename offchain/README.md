...
If you want to develop for this submodule, please before setting up an
environment consult the notes on [CONTRIBUTING.md](CONTRIBUTING.md) first.
...
To change the testnet you're using, you have to change the network name in the
`flake.nix`
...
You can also run these components directly without using Docker, more about
these can be found
[here](https://github.com/Plutonomicon/cardano-transaction-lib/blob/develop/doc/runtime.md).
...
configure it via CLI arguments, or set these in the configuration.
...
For more information about the arguments, please refer to `nix run
.#sidechain-main-cli -- burn-v1 --help`
...
You can call the contract endpoints with the following CLI command (you need to
...
```json
{
  ...
  "stakeSigningKeyFile": "/absolute/path/to/stake.skey"
}
```
...
Notes:
  ...
  - If not using a config file, prior to running the contracts it may be
    desirable to have available your signing key in the environment. Example:
...
```
  init-tokens-mint         Pre-mint tokens without setting the initial committee
...
Normally, the protocol has to wait until the committee has reached the desired
number of members.  To defend against accidentally consuming the
`genesis-committee-hash-utxo`, it's possible, to split up the initialisation
step into two separate commands:

1. mint tokens identifying tokens used later in the protocol (spending the
   `genesis-committee-hash-utxo`)
2. set up the first committee (only the owner of the above minted tokens can do
   this)
...
Alternatively, one may optionally mint candidate permission tokens in the same
transaction as the `init-tokens-mint` transaction. The following command
achieves this.
...
#### 3.3.8 Candidiate permission token
...
#### 3.3.8 Insert new protocol version
...
#### 3.3.9 Update existing protocol version
...
#### 3.3.10 Invalidate protocol version
...
#### 3.3.11 Insert a D parameter value
...
#### 3.3.12 Update a D parameter value
...
#### 3.3.13 Remove a D parameter value
...
#### 3.3.14 Update a list of permissioned candidates
