...
main =
  defaultMain . adjustOption go . testGroup "Roundtrip" $
    ...
    , testProperty "SignedMerkleRootMint (safe)" . toDataSafeLaws' genSMRM shrinkSMRM $ show
    , testProperty "SignedMerkleRootMint (unsafe)" . toDataUnsafeLaws' genSMRM shrinkSMRM $ show
    ...
    , testProperty "FUELRedeemer (safe)" . toDataSafeLaws' genFR shrinkFR $ show
    , testProperty "FUELRedeemer (unsafe)" . toDataUnsafeLaws' genFR shrinkFR $ show
    , testProperty "FUELMint (safe)" . toDataSafeLaws' genFM shrinkFM $ show
    , testProperty "FUELMint (unsafe)" . toDataUnsafeLaws' genFM shrinkFM $ show
...
genFR :: Gen FUELRedeemer
genFR = oneof [MainToSide <$> go, SideToMain <$> genMTE <*> genMP]
  where
    go :: Gen LedgerBytes
    go = do
      EcdsaSecp256k1PubKey lb <- genPK
      pure lb
...
genCCM = do
  ArbitraryCurrencySymbol cop <- arbitrary
  Positive tn <- arbitrary
  Positive td <- arbitrary
  pure . CommitteeCertificateMint cop tn $ td
...
genFM = do
  ArbitraryCurrencySymbol rtcs <- arbitrary
  sp <- genSP
  ArbitraryCurrencySymbol dkcs <- arbitrary
  pure . FUELMint rtcs sp $ dkcs
...
genSMRM = do
  ...
  ArbitraryValidatorHash vh <- arbitrary
  pure . SignedMerkleRootMint sp cs $ vh
...
genSP = do
  ...
  pure . SidechainParams cid gh gu n $ d
...
shrinkFR :: FUELRedeemer -> [FUELRedeemer]
shrinkFR = \case
  MainToSide lb ->
    MainToSide <$> do
      EcdsaSecp256k1PubKey lb' <- shrinkPK (EcdsaSecp256k1PubKey lb)
      pure lb'
  SideToMain mte mp -> SideToMain <$> shrinkMTE mte <*> shrinkMP mp
...
shrinkCCM (CommitteeCertificateMint {..}) = do
  ArbitraryCurrencySymbol cop' <- shrink (ArbitraryCurrencySymbol committeeOraclePolicy)
...
shrinkFM :: FUELMint -> [FUELMint]
shrinkFM (FUELMint {..}) = do
  ArbitraryCurrencySymbol rtcs' <- shrink (ArbitraryCurrencySymbol mptRootTokenCurrencySymbol)
  sp' <- shrinkSP sidechainParams
  ArbitraryCurrencySymbol dkcs' <- shrink (ArbitraryCurrencySymbol dsKeyCurrencySymbol)
  pure . FUELMint rtcs' sp' $ dkcs'

shrinkSMRM :: SignedMerkleRootMint -> [SignedMerkleRootMint]
shrinkSMRM (SignedMerkleRootMint {..}) = do
  sp' <- shrinkSP sidechainParams
  ArbitraryCurrencySymbol cs' <- shrink (ArbitraryCurrencySymbol committeeCertificateVerificationCurrencySymbol)
...
shrinkSP (SidechainParams {..}) = do
  ...
  pure . SidechainParams cid' gh' gu' n' $ thresholdDenominator
