...
serialiseScriptsToPurescript moduleName plutusScripts handle = do
  ...
  IO.hPutStrLn handle "--   to update `src/TrustlessSidechain/RawScripts.purs` and `test/Test/PoCRawScripts.purs`."
  IO.hPutStrLn handle "-- Note: if the modified times do not accurately capture an out of date"
  IO.hPutStrLn handle "-- `src/TrustlessSidechain/RawScripts.purs`  and `test/Test/PoCRawScripts.purs`,"
  IO.hPutStrLn handle "-- then run `make clean` before running `make update-scripts`."
     ...
     in case options of
          ...
          GenPureScriptPoCRawScripts {gpspocrsOutputFile = outputFile} ->
            let moduleName = "Test.PoCRawScripts"
             in case outputFile of
                  Nothing -> serialiseScriptsToPurescript moduleName plutusPoCScripts IO.stdout
                  Just filepath ->
                    IO.withFile filepath IO.ReadWriteMode $ \handle ->
                      -- clear the file first, then put our code in.
                      IO.hSetFileSize handle 0
