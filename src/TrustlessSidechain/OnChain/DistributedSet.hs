...
import Plutus.V2.Ledger.Api -- (CurrencySymbol , OutputDatum(..) , ValidatorHash , Address , Validator , MintingPolicy , TokenName , Value , Map , mkMintingPolicyScript , getValue)
import Plutus.Script.Utils.V2.Scripts(scriptCurrencySymbol , mkUntypedMintingPolicy , validatorHash , mkUntypedValidator)
...
unsafeGetDatum info o = case txOutDatum o of
  OutputDatumHash dhash | Just bn <- Contexts.findDatum dhash info ->
...
mkInsertValidator ds _dat _red ctx =
  ( \nStr ->
      ( \nNodes ->
        ...
        (fromMaybe (traceError "error 'mkInsertValidator' bad insertion")
...
insertValidator ds = let
  wrap = \d -> mkUntypedValidator (mkInsertValidator d)
...
dsConfPolicy dscm = mkMintingPolicyScript
...
dsKeyPolicy dskm = let
  wrap d = mkUntypedMintingPolicy (mkDsKeyPolicy d)
