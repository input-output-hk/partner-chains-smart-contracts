...
- `FUELMintingPolicy`: minting policy validating the minting or burning of FUEL tokens on mainchain ([2.](#2-transfer-fuel-tokens-from-mainchain-to-sidechain), [3.2.](#32-individual-claiming))
...
2. The committee certificate verification minting policy is parameterized
   ...
   The committee certificate verification minting policy mints only if:
    ...
    - `tn` has *enough* committee members that have verified `tn` from the
